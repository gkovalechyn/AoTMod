...
public class FengGameManagerMKII : Photon.MonoBehaviour {
    //--
    private PhotonPlayerComparer photonPlayerComparer = new PhotonPlayerComparer();
    //--
    ...
    public static HashSet<int> ignoreList;
    ...
    private Dictionary<string, float> racingResult;
    ...
    public int wave = 1;
    public Boolean novaMenu = false;

    public int checkLag;

    public new string name { get; set; }

    public FengGameManagerMKII() {
        if (AoTModAPI.AoTModAPI.getModAPI() == null) {
            AoTModAPI.AoTModAPI.initialize(this);
        }
    }
    ...
    private void Chat(string content, string sender, PhotonMessageInfo info) {
        AoTModAPI.AoTModAPI.getModAPI().onChatReceived(sender, content, info);
    ...
    private bool checkIsTitanAllDie() {
        ...
        foreach(FEMALE_TITAN ft in this.fT) {
    ...
    private void getRacingResult(string player, float time, PhotonMessageInfo info) {
        if (!this.racingResult.ContainsKey(player)) {
            if (player != null && time > 0) {
                this.racingResult[player] = time;
    ...
    private void ignorePlayer(int ID, PhotonMessageInfo info) {
        if (info.sender.isMasterClient && ID != PhotonNetwork.player.ID) {
            //Needs to rejoin to get an new id if it's local
    ...
    public void justRecompileThePlayerList() {
        ...
        builder.Append("[000000]Public Security mod[-]\n");
        ...
        if (RCSettings.teamMode != 0) {
            ...
            builder.Append("[00FFFF]TEAM CYAN[ffffff]:").Append(this.cyanKills).Append("/").Append(num3).Append("/").Append(num5).Append("/").Append(num7).Append("\n");

            foreach (PhotonPlayer player in dictionary.Values) {
                num10 = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.RCteam]);
                if (num10 == 1) {
                    this.generateStringFromPlayer(player, builder);
                    builder.Append("\n");
                }
            }
            builder.Append("[FF00FF]TEAM MAGENTA[ffffff]:").Append(this.cyanKills).Append("/").Append(num4).Append("/").Append(num6).Append("/").Append(num8).Append("\n");
            foreach (PhotonPlayer player in dictionary2.Values) {
                num10 = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.RCteam]);
           ...
           builder.Append(" \n[00FF00]INDIVIDUAL\n");
            ...
            foreach (PhotonPlayer player in dictionary3.Values) {
                this.generateStringFromPlayer(player, builder);
                builder.Append("\n");
                 
            }
        } else {
            this.generateStringFromPlayer(PhotonNetwork.player, builder);
            builder.Append("\n");

            //It returns a new array
            PhotonPlayer[] otherPlayers = new PhotonPlayer[PhotonNetwork.otherPlayers.Length];

            Array.Sort(otherPlayers, this.photonPlayerComparer);
        ...
        int kills = RCextensions.returnIntFromObject(PhotonNetwork.player.customProperties[PhotonPlayerProperty.kills]);
        int damage = RCextensions.returnIntFromObject(PhotonNetwork.player.customProperties[PhotonPlayerProperty.total_dmg]);
        this.playerList = builder.Append(AoTModAPI.AoTModAPI.getModAPI().generateStatsString(kills, damage)).ToString();
    ...
    public void kickPlayerRC(PhotonPlayer player, bool ban, string reason) {
        ...
        if (OnPrivateServer) {
            ...
            foreach (PhotonPlayer p in PhotonNetwork.playerList) {
                if (p.ID != player.ID) {
                    base.photonView.RPC("ignorePlayer", player, new object[] { player.ID });
                }
            }

            //mod
            if (ban && !ignoreList.Contains(player.ID)) {
                string stripped = AoTModAPI.Util.stripColorCodes((string)player.customProperties[PhotonPlayerProperty.name]);
                AoTModAPI.AoTModAPI.getModAPI().banName(stripped, 0);
            }
    ...
    private void LateUpdate() {
        AoTModAPI.AoTModAPI.getModAPI().onLateUpdate();
                ...
                while (enumerator3.MoveNext()) {
                    ...
                    if (current != null) {
                        try {
                            ...
                            AoTModAPI.AoTModAPI.getModAPI().log(e);
                        }
                    }
                }
            } finally {
    ...
    private void netGameLose(int score, PhotonMessageInfo info) {
        if (!((info.sender == PhotonNetwork.masterClient) || info.sender.isLocal) && PhotonNetwork.isMasterClient) {
            ...
            //this.kickPlayerRC(info.sender, false, "False netGameLose");
            return;
        }
    ...
    private void netGameWin(int score, PhotonMessageInfo info) {
        if (!((info.sender == PhotonNetwork.masterClient) || info.sender.isLocal) && PhotonNetwork.isMasterClient) {
            this.chatRoom.addLINE("<color=#FFC000>Round end sent from Player " + info.sender.ID + "</color>");
            //this.kickPlayerRC(info.sender, false, "False netGameWin");
            return;
        }
    ...
    public void OnCreatedRoom() {
        ...
        this.racingResult = new Dictionary<string, float>();
    ...
    public void OnGUI() {
        ...
        if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.STOP) && (Application.loadedLevelName != "characterCreation")) {
            if (isAssetLoaded) {/*
                ...
                //Making a new GUI for the main menu
//                GUI.Button(new Rect(Screen.width - 400, 220, 400, 200), "Pixel");
                GUI.Box(new Rect(Screen.width-500, Screen.height+400, 150f, 105f), string.Empty);
                    ...
                    if (text.StartsWith("Verifying")) {
                        ...
                        if (((int)settings[0xbb]) == 1) {
                            if (loginstate == 3) {
                                ...
                                nameField = GUI.TextField(new Rect(65f, 80f, 145f, 20f), nameField, 120);
                                GUI.Label(new Rect(20f, 105f, 45f, 20f), "Guild:", "Label");
                                LoginFengKAI.player.guildname = GUI.TextField(new Rect(65f, 105f, 145f, 20f), LoginFengKAI.player.guildname, 120);
                            ...
                            if (loginstate == 3) {
                                ...
                                GUI.Label(new Rect(20f, 80f, 70f, 20f), "Username:", "Label");
                                usernameField = GUI.TextField(new Rect(90f, 80f, 130f, 20f), usernameField, 80);
                                GUI.Label(new Rect(20f, 105f, 70f, 20f), "Password:", "Label");
                                passwordField = GUI.PasswordField(new Rect(90f, 105f, 130f, 20f), passwordField, '*', 80);
            ...
            if (((int)settings[0x40]) >= 100) {
                ...
                if (Time.timeScale <= 0.1f) {
                    ...
                    if (length >= num50) {
                        logicLoaded = customLevelLoaded = true;
                    }
                }
            }
        }
    ...
    public void OnJoinedRoom() {
        ...
        NetworkingPeer.lastJoinRoomMs = (long) (DateTime.UtcNow - new DateTime(1970, 1, 1)).TotalMilliseconds;
        AoTModAPI.AoTModAPI.getModAPI().onJoinedRoom();
        ...
        //hashtable.Add(PhotonPlayerProperty.modProperty, 0);
    ...
    private void OnLevelWasLoaded(int level) {
        ...
        //--MOD
        AoTModAPI.AoTModAPI.getModAPI().onLevelWasLoaded(LevelInfo.getInfo(FengGameManagerMKII.level));
        //--

    }

    public void OnMasterClientSwitched(PhotonPlayer newMasterClient) {
        AoTModAPI.AoTModAPI.getModAPI().onMasterClientChanged(newMasterClient.ID);

        if (newMasterClient.ID == PhotonNetwork.player.ID) {
            PhotonNetwork.networkingPeer.resetRPCHistory();
            NetworkingPeer.lastJoinRoomMs = (long)(DateTime.UtcNow - new DateTime(1970, 1, 1)).TotalMilliseconds;
        }
        ...
        if (PhotonNetwork.isMasterClient) {
            ...
            if (banHash.ContainsValue(RCextensions.returnStringFromObject(player.customProperties[PhotonPlayerProperty.name]))) {
                ...
                if ((ignoreList != null) && (ignoreList.Count > 0)) {
                    int[] array = new int[ignoreList.Count];
                    ignoreList.CopyTo(array, 0, array.Length);
                    photonView.RPC("ignorePlayerArray", player, new object[] { array});
                }
    ...
    public void OnPhotonPlayerDisconnected(PhotonPlayer player) {
        if (!this.gameTimesUp) {
            ...
            this.someOneIsDead(0, new PhotonMessageInfo());
        }
    ...
    public void pauseRPC(bool pause, PhotonMessageInfo info) {
        AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Cancelled the pausing of the game.");
        return;
        if (info.sender.isMasterClient) {
            if (pause) {
    ...
    private void refreshPVPStatus(int score1, int score2, PhotonMessageInfo info) {
        /*
        if (!info.sender.isMasterClient) {
             AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to set the PVP status");
            if (PhotonNetwork.isMasterClient) {
                this.kickPlayerRC(info.sender, false, "Tried to refresh the PVP status");
            }
        }
         * */
    ...
    private void refreshStatus(int score1, int score2, int wav, int highestWav, float time1, float time2, bool startRacin, bool endRacin, PhotonMessageInfo info) {
        if (!info.sender.isMasterClient) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player  #" + info.sender.ID + " tried to refresh the status.");

            if (PhotonNetwork.isMasterClient) {
                this.kickPlayerRC(info.sender, false, "Tried to force refresh the status.");
            }
    ...
    private void respawnHeroInNewRound(PhotonMessageInfo info) {
        if (!info.sender.isMasterClient) {
            if (PhotonNetwork.isMasterClient) {
                this.kickPlayerRC(info.sender, false, "Non-MC respawn.");
            }
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Received respawn from player #" + info.sender.ID);
        }
    ...
    private void restartGameByClient(PhotonMessageInfo info) {
        AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to restart the game (CLIENT)");
    }
...
Label_095F:
    ...
    private void RPCLoadLevel(PhotonMessageInfo info) {
        if (info.sender.isMasterClient) {
            AoTModAPI.AoTModAPI.getModAPI().onLevelReload();
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Master client reloaded the level.");
            this.DestroyAllExistingCloths();
            PhotonNetwork.LoadLevel(LevelInfo.getInfo(level).mapName);
        } else if (PhotonNetwork.isMasterClient && info.sender != PhotonNetwork.player) {
            AoTModAPI.AoTModAPI.getModAPI().banName(AoTModAPI.Util.stripColorCodes((string)info.sender.customProperties[PhotonPlayerProperty.name]), 0);
            this.kickPlayerRC(info.sender, true, "false restart.");
            return;
        } else if (!masterRC) {
            AoTModAPI.AoTModAPI.getModAPI().log("Player #" + info.sender + " force reloaded the level.");
    ...
    private void showChatContent(string content, PhotonMessageInfo info) {
        AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " sent the showChatContent RPC");
        return;
        /*
    ...
    private void showResult(string text0, string text1, string text2, string text3, string text4, string text6, PhotonMessageInfo info) {
        if (PhotonNetwork.isMasterClient && info.sender != PhotonNetwork.player) {
            this.kickPlayerRC(info.sender, true, "False game end.");
            AoTModAPI.AoTModAPI.getModAPI().banName(AoTModAPI.Util.stripColorCodes((string)info.sender.customProperties[PhotonPlayerProperty.name]), AoTModAPI.ModBanType.FULL_NAME);
        } else {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer(((info.sender.isMasterClient) ? "[MC]" : "") + "Player #" + info.sender.ID + " tried to end the game.");
        }
    ...
    public GameObject spawnTitan(int rate, Vector3 position, Quaternion rotation, bool punk = false) {
        object result = AoTModAPI.AoTModAPI.getModAPI().spawnTitanHook(rate, position, rotation, punk);
        if (result != null) {
    ...
    public void titanGetKill(PhotonPlayer player, int Damage, string name, PhotonMessageInfo info) {
        if (checkLag >= 10 || Damage >= 6969) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " is sending too many titanGetKill RPCs, or damage too high. Damage: " + Damage);
            return;
        }

        if (!info.sender.isMasterClient) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " sent a fake titanGetKill as player #" + player.ID);

            if (PhotonNetwork.isMasterClient) {
                this.kickPlayerRC(player, true, "Fake titanGetKill");
                string stripped = AoTModAPI.Util.stripColorCodes((string)info.sender.customProperties[PhotonPlayerProperty.name]);
                AoTModAPI.AoTModAPI.getModAPI().banName(stripped,  AoTModAPI.ModBanType.FULL_NAME);
            }
            return;
        }
    ...
    private IEnumerator checkingLagE() {
        checkLag++;
        yield return new WaitForSeconds(1f);
        checkLag = 0;
    }

    public void checkLagM() {
        base.StartCoroutine(this.checkingLagE());
    }
    ...
    private void Update() {
        ...
        if (this.gameStart) {
            ...
            try {
                while (enumerator4.MoveNext()) {
                    titan = (TITAN)enumerator4.Current;
                    if (titan != null)
                        titan.update2();
                }
                ...
                this.titans.Remove(titan);
                PhotonNetwork.Destroy(titan.photonView);
                this.oneTitanDown("", false);
                //AoTModAPI.AoTModAPI.getModAPI().onTitanDown();
            } finally {
                IDisposable disposable4 = enumerator4 as IDisposable;
                if (disposable4 != null) {
                    disposable4.Dispose();
    ...
    private void updateKillInfo(bool t1, string killer, bool t2, string victim, int dmg, PhotonMessageInfo info) {
        if (PhotonNetwork.isMasterClient && dmg >= 6969) {
            this.kickPlayerRC(info.sender, false, "Fake damage (>=6969)");
            return;
        }
        ...
        obj3 = (GameObject)UnityEngine.Object.Instantiate(Resources.Load("UI/KillInfo"));
        ...
        if (killer != null && killer.Equals(global::PhotonNetwork.player.customProperties[global::PhotonPlayerProperty.name])) {
            AoTModAPI.AoTModAPI.getModAPI().playerKilledTitan(dmg);
            return;
        }

        if (victim != null && victim.Equals(PhotonNetwork.player.customProperties[global::PhotonPlayerProperty.name])) {
            AoTModAPI.AoTModAPI.getModAPI().playerDied();
        }
    }
    ...
    private void generateStringFromPlayer(PhotonPlayer player, StringBuilder builder) {
        if (!player.customProperties.ContainsKey(PhotonPlayerProperty.dead)) {
            builder.Append("[FF0000]DEAD IS NULL[-]");
        }

        if (ignoreList.Contains(player.ID)) {
            builder.Append("[FF0000][X] ");
        }
        if (player.isLocal) {
            builder.Append("[00CC00]");
        } else {
            builder.Append("[FFCC00]");
        }

        builder.Append("[" + Convert.ToString(player.ID) + "] ");

        if (player.isMasterClient) {
            builder.Append("[00BB00][M] ");
        }
        //=----
        if (player.ID != PhotonNetwork.player.ID) {
            getMods(builder, player);
        }
        //=----

        if (RCextensions.returnBoolFromObject(player.customProperties[PhotonPlayerProperty.dead])) {
            builder.Append("[" + ColorSet.color_red + "] *dead* ");
        }
        if (RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.isTitan]) < 2) {
            int team = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.team]);

            if (team < 2) {
               builder.Append("[").Append(ColorSet.color_human).Append("] <H> ");
            } else if (team == 2) {
                builder.Append("[").Append(ColorSet.color_human_1).Append("] <A> ");
            }
        } else if (RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.isTitan]) == 2) {
            builder.Append("[").Append(ColorSet.color_titan_player).Append("] <T> ");
        }

        string name = RCextensions.returnStringFromObject(player.customProperties[PhotonPlayerProperty.name]);
        int kills = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.kills]);
        int deaths = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.deaths]);
        int maxDamage = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.max_dmg]);
        int totalDamage = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.total_dmg]);

        builder.Append(name).Append("[ffffff]:").Append(kills).Append("/").Append(deaths).Append("/").Append(maxDamage).Append("/").Append(totalDamage);
    ...
    public IEnumerator WaitAndRecompilePlayerList(float time) {
        ...
        if (RCSettings.teamMode == 0) {
            ...
            builder.Append("[00FFFF]TEAM CYAN[ffffff]:").Append(this.cyanKills).Append("/").Append(num4).Append("/").Append(num6).Append("/").Append(num8).Append("\n");
            foreach (PhotonPlayer player in dictionary.Values) {
                num11 = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.RCteam]);
            ...
            builder.Append("[FF00FF]TEAM MAGENTA[ffffff]:").Append(this.magentaKills).Append("/").Append(num5).Append("/").Append(num7).Append("/").Append(num9).Append("\n");
            foreach (PhotonPlayer player in dictionary2.Values) {
                num11 = RCextensions.returnIntFromObject(player.customProperties[PhotonPlayerProperty.RCteam]);
    ...
    public void getMods(StringBuilder builder, PhotonPlayer playeri) {
        bool modded = false;
		//I should probably make this so that I don't have to recompile it every time, but fuck it
        if (playeri.customProperties.ContainsKey("raohsopmod")) {
            builder.Append("[FFCC00][RM]");
            modded = true;
        }
        if (playeri.customProperties.ContainsKey("GHOST")) {
            builder.Append("[0000FF][GH]");
            modded = true;
        }
        if (playeri.customProperties.ContainsKey("NRC")) {
            builder.Append("[00FFFF][NRC]");
            modded = true;
        }

        if (playeri.customProperties.ContainsKey("SRC")) {
            builder.Append("[FFFF00][SRC]");
            modded = true;
        }
        if (playeri.customProperties.ContainsKey("Nathan")) {
           builder.Append("[000000][A&N]");
            modded = true;
        }
        if (playeri.customProperties.ContainsKey("KageNoKishi")) {
            builder.Append("[800080][KnK]");
            modded = true;
        }
        if (playeri.customProperties.ContainsKey("Angry_Guest")) {
            builder.Append("[C0c0c0][PIX]");
            modded = true;
        }

        if (playeri.customProperties.ContainsKey("CyanMod")) {
            builder.Append("[00FFFF][CyM]");
            modded = true;
        }

        if (playeri.customProperties.ContainsKey("CyanModNew")) {
            builder.Append("[00FFFF][CyMN]");
            modded = true;
        }

        if (playeri.customProperties.ContainsKey("OhSoShO")) {
            builder.Append("[FF0000][OSS]");
            modded = true;
        }
        if (playeri.customProperties.ContainsKey("Cear")) {
            builder.Append("[646499][CM]");
            modded = true;
        }

        if (playeri.customProperties.ContainsKey("Arch")) {
            builder.Append("[696969][Arch]");
            modded = true;
        }

        if (playeri.customProperties.ContainsKey("REDSKIES")) {
            builder.Append("[FF0000][RS]");
            modded = true;
        }

        if (!modded && AoTModAPI.AoTModAPI.getModAPI().containsDifferentProperty(playeri.ID)) {
            builder.Append("[FFFF00][!!!]");
        }

        if (!modded && playeri.customProperties.ContainsKey("RCteam")) {
            builder.Append("[9999FF][RC]");
        }
    }
    ...
    private class PhotonPlayerComparer : IComparer<PhotonPlayer> {
