...
public class Bullet : Photon.MonoBehaviour
{
    ...
    private void killObject(PhotonMessageInfo info)
    {
        if (this.photonView.isMine && !info.sender.isLocal) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to delete your hooks.");
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, false, "Tried to delete your hooks");
            }
            return;
        }
    ...
    private void myMasterIs(int id, string launcherRef, PhotonMessageInfo info){
        ...
        if (view != null && view.GetComponent<HERO>() != null) {
            this.master = view.gameObject;
            ...
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " sent an invalid viewID for the hook master.");
            if (PhotonNetwork.isMasterClient) {
                //FengGameManagerMKII.instance.kickPlayerRC(info.sender, false, "Invalid hook master ID");
            }
        }
    }
    ...
    private void setLinePhase0()
    {
        ...
        else if (this.nodes.Count > 0 && this.myRef != null && this.lineRenderer != null)
        {
    ...
    private void tieMeTo(Vector3 p, PhotonMessageInfo info){
        if (this.photonView.isMine && info.sender != this.photonView.owner) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to tie your hook to something.");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "RPC call for your object. (Bullet::tieMeTo)");
            }
            return;
        }
    ...
    private void tieMeToOBJ(int id, PhotonMessageInfo info){
        if (this.photonView.isMine && info.sender != this.photonView.owner) {
            ...
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "RPC call for your object. (Bullet::tieMeToOBJ)");
            }
            return;
        }
