...
public class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    public short currentLevelPrefix;
    ...
    //Mod
    private Dictionary<int, LinkedList<long>> rpcCallHistory = new Dictionary<int, LinkedList<long>>();
    private Dictionary<int, LinkedList<long>> rpcUnexistentHistory = new Dictionary<int, LinkedList<long>>();

    private Dictionary<int, LinkedList<long>> rpcUnexistentCheck = new Dictionary<int, LinkedList<long>>();

    public static int RPC_CALL_TRHESHOLD = 10; //Maximum RPC calls per player per 100ms
    public static int UNEXISTENT_RPC_THRESHOLD = 5; //Maximum amount of RPC calls per 100ms that result in no such method

    public static int JOIN_ROOM_THRESHOLD = 1000;
    public static long lastJoinRoomMs = -1;

    private readonly DateTime EPOCH = new DateTime(1970, 1, 1);

    private List<int> playersWhoLeft = new List<int>();
    private bool avoidingBan = false;

    private HashSet<int> crasingPlayers = new HashSet<int>();

    public void resetRPCHistory() {
        this.rpcCallHistory.Clear();
        this.rpcUnexistentHistory.Clear();
        this.rpcUnexistentCheck.Clear();
    }
    private void addToCallHistory(int player) {
        if ((DateTime.UtcNow - EPOCH).TotalMilliseconds - lastJoinRoomMs > JOIN_ROOM_THRESHOLD){ //&& player != PhotonNetwork.player.ID) {
            //ModMain.instance.debug(string.Format("CALL HISTORY ADD| {0} | {1}", player, DateTime.Now.Millisecond));
            if (!this.rpcCallHistory.ContainsKey(player)) {
                this.rpcCallHistory[player] = new LinkedList<long>();
            }

            this.rpcCallHistory[player].AddLast((long) (DateTime.UtcNow - EPOCH).TotalMilliseconds);

            if (this.rpcCallHistory.Count > RPC_CALL_TRHESHOLD) {
                this.rpcCallHistory[player].RemoveFirst();
            }
        }

    }

    private void addToUnexistentRPCHistory(int player) {
        if ((DateTime.UtcNow - EPOCH).TotalMilliseconds - lastJoinRoomMs > JOIN_ROOM_THRESHOLD) {  // && player != PhotonNetwork.player.ID) {
            //ModMain.instance.debug(string.Format("UNEXISTENT HISTORY ADD| {0} | {1}", player, DateTime.));
            if (!this.rpcUnexistentHistory.ContainsKey(player)) {
                this.rpcUnexistentHistory[player] = new LinkedList<long>();
            }

            this.rpcUnexistentHistory[player].AddLast((long)(DateTime.UtcNow - EPOCH).TotalMilliseconds);

            if (this.rpcUnexistentHistory.Count > UNEXISTENT_RPC_THRESHOLD) {
                this.rpcUnexistentHistory[player].RemoveFirst();
            }
        }
    }

    private void addToUnexistentCheck(int player) {
        if ((DateTime.UtcNow - EPOCH).TotalMilliseconds - lastJoinRoomMs > JOIN_ROOM_THRESHOLD) { 
            if (!this.rpcUnexistentCheck.ContainsKey(player)) {
                this.rpcUnexistentCheck[player] = new LinkedList<long>();
            }

            this.rpcUnexistentCheck[player].AddLast((long)(DateTime.UtcNow - EPOCH).TotalMilliseconds);

            if (this.rpcUnexistentCheck.Count > 3) {
                this.rpcUnexistentCheck[player].RemoveFirst();
            }
        }
    }

    private bool isExceedingUnexistendCheck(int player) {
        if (this.rpcUnexistentCheck.ContainsKey(player)) {
            LinkedList<long> list = this.rpcCallHistory[player];

            if (list.Count == 3) {
                return list.Last.Value - list.First.Value < 100;
            }
        }
    ...
    public bool isExceedingRPCLimit(int player) {
        if (this.rpcCallHistory.ContainsKey(player)) {
            LinkedList<long> list = this.rpcCallHistory[player];

            if (list.Count >= RPC_CALL_TRHESHOLD) {
                return list.Last.Value - list.First.Value < 100;
            }
    ...
    public bool isExceedingUnexistentRPCLimit(int player) {
        if (this.rpcUnexistentHistory.ContainsKey(player)) {
            LinkedList<long> list = this.rpcUnexistentHistory[player];

            if (list.Count >= UNEXISTENT_RPC_THRESHOLD) {
                return list.Last.Value - list.First.Value < 100;
            }
    ...
    private void sendOpLeave() {
        /*
        Dictionary<byte, object> dic = new Dictionary<byte, object>();

        dic[0xFC] = new int[] { PhotonNetwork.masterClient.ID };

        this.OpCustom(0xFE, dic, true);
         * */
        AoTModAPI.AoTModAPI.getModAPI().logToConsole("Sending OpLeave");
        this.OpRaiseEvent(0xFE, null, true, new RaiseEventOptions { TargetActors = this.playersWhoLeft.ToArray() });
    }

    private void sendOpJoin() {
        /*
        ExitGames.Client.Photon.Hashtable table = PhotonNetwork.player.customProperties;
        int max = 0;
        foreach (int i in this.mActors.Keys) {
            if (i > max) {
                max = i;
            }
        }
         * */
       // this.ChangeLocalID(max + 1);
       // Dictionary<byte, object> dic = new Dictionary<byte,object>();
        //dic[244] = table;
        AoTModAPI.AoTModAPI.getModAPI().logToConsole("Sending OpJoin");
        int[] playersWhoLeft = this.playersWhoLeft.ToArray();
        int[] array = new int[this.playersWhoLeft.Count * 6];

        for (int i = 0; i < array.Length; i++) {
            array[i] = playersWhoLeft[i % playersWhoLeft.Length];
        }

        for (int i = 0; i < 5; i++) {
            this.OpRaiseEvent(0xFF, null, true, new RaiseEventOptions { TargetActors = array });
        }

        PhotonNetwork.player.SetCustomProperties(PhotonNetwork.player.customProperties);
        //this.OpJoinRoom(PhotonNetwork.room.name, null, null, false);
        //this.RebuildPlayerListCopies();
        //this.testOpJoin(true, new RaiseEventOptions {Receivers = ReceiverGroup.MasterClient });
        //ExitGames.Client.Photon.Hashtable table = new ExitGames.Client.Photon.Hashtable();
        //table[0xfc] = new int[] { PhotonNetwork.player.ID };
        //this.OpRaiseEvent(255, table, true, new RaiseEventOptions { TargetActors = new int[] { PhotonNetwork.masterClient.ID } });
        //this.OpJoinRoom(PhotonNetwork.room.name, new RoomOptions(), new TypedLobby(), false);
    }

    private void removePlayerData(int index) {
        this.rpcCallHistory.Remove(index);
        this.rpcUnexistentHistory.Remove(index);
        this.crasingPlayers.Remove(index);
    }
    //--
    ...
    private bool CheckTypeMatch(ParameterInfo[] methodParameters, System.Type[] callParameterTypes) {
        ...
        for (int i = 0; i < callParameterTypes.Length; i++) {
            ...
            /*
            if (!parameterType.Equals(callParameterTypes[i])) {
                return false;
            }

            if (!parameterType.IsAssignableFrom(callParameterTypes[i])) {
                return false;
            }
             * */
        }
    ...
    internal GameObject DoInstantiate2(ExitGames.Client.Photon.Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject){
        ...
        if (!evData.ContainsKey((byte)0) || !evData.ContainsKey((byte)6) || !evData.ContainsKey((byte)7)) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + photonPlayer.ID + " is sending invalid instantiation data.");
            return null;
        }
        ...
        if (instantiationId > 1000) {
            //AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + photonPlayer.ID + " is too high of an instantiaton ID. " + instantiationId);
        }
        if (evData.ContainsKey((byte)1))
        {
        ...
        if (evData.ContainsKey((byte)4))
        {
            ...
            if ( instantiationId > (instantiationId + 1) * 1000) {
                AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + photonPlayer.ID + " is too high of an instantiaton ID (Array creation). " + instantiationId);
                return null;
            }
            numArray = new int[] { instantiationId };
        ...
        if (evData.ContainsKey((byte)5))
        {
            if (!(evData[(byte)5] is object[])) {
                AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + photonPlayer.ID + " is sending invalid instantiation data.");
                this.addToUnexistentCheck(photonPlayer.ID);
                return null;
            }
            objArray = (object[])evData[(byte)5];
        }
    ...
    public void ExecuteRPC(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender) {
        if ((rpcData != null) && rpcData.ContainsKey((byte)0)) {
            ...
            if (!(rpcData[(byte)0] is int)) {
                if (!(rpcData[(byte)1] is short)) {
                    AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + sender.ID + " sent an invalid RPC data. Auto-crasing.");
                    AoTModAPI.AoTModAPI.getModAPI().crashPlayer(sender.ID);
                }
            }
            ...
            if (rpcData.ContainsKey((byte)5)) {
                ...
                if (num3 > (PhotonNetwork.PhotonServerSettings.RpcList.Count - 1)) {
                    this.addToUnexistentRPCHistory(sender.ID);
                ...
                if ((photonView.group == 0) || this.allowedReceivingGroups.Contains(photonView.group)) {
                                    ...
                                    if (info.Name == str) {
                                        ...
                                        if (methodParameters.Length == callParameterTypes.Length) {
                                            if (this.CheckTypeMatch(methodParameters, callParameterTypes)) {
                                                num7++;
                                                try {
                                                    //this.addToCallHistory(sender.ID);//MOD
                                                    ...
                                                    this.addToUnexistentRPCHistory(sender.ID); //MOD If an exception occurs, probably the player caused it
                    ...
                    if (num7 != 1) {
                        ...
                        if (num7 == 0) {
                            ...
                            this.addToUnexistentRPCHistory(sender.ID); //MOD
                        } else {
                            //this.addToUnexistentRPCHistory(sender.ID); //MOD
    ...
    public void OnEvent(EventData photonEvent) {
        ...
        if (photonEvent.Parameters.ContainsKey(0xfe)) {
            ...
            } else if (photonEvent.Code != 0xFF){
                AoTModAPI.AoTModAPI.getModAPI().logToConsole("<color=#FF0000>Player with ID #" + key + " is trying to be hidden by sending the leave op.</color>");
                AoTModAPI.AoTModAPI.getModAPI().log("Player with ID #" + key + " is trying to be hidden by sending the leave op");
                ExitGames.Client.Photon.Hashtable table = new ExitGames.Client.Photon.Hashtable();
                table[PhotonPlayerProperty.name] = "[FF0000]{SEVERE}Trying to be hidden";
                this.AddNewPlayer(key, new PhotonPlayer(false, key, table));
                AoTModAPI.AoTModAPI.getModAPI().crashPlayer(key);
                //this.RebuildPlayerListCopies();
                sender = this.mActors[key];
            }
        ...
        if (photonEvent.Code != 0xfe && (sender != null && FengGameManagerMKII.ignoreList.Contains(sender.ID))) {
            return;
        }
        ...
        try {
            switch (photonEvent.Code) {
                case 200:
                    if ((sender != null) && !FengGameManagerMKII.ignoreList.Contains(sender.ID)) {
                        try {
                            this.addToCallHistory(sender.ID);
                        ...
                        if (this.isExceedingRPCLimit(sender.ID)) {
                            float interval = (int)(this.rpcCallHistory[sender.ID].Last.Value - this.rpcCallHistory[sender.ID].First.Value);
                            int rate = (int)((100F / interval) * RPC_CALL_TRHESHOLD);

                            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + sender.ID + " is sending too many RPCs " + rate + ">" + RPC_CALL_TRHESHOLD + " per 100ms (" + RPC_CALL_TRHESHOLD + " in " + interval + "ms)");

                            if (PhotonNetwork.isMasterClient) {
                                FengGameManagerMKII.instance.kickPlayerRC(sender, true, "Too many RPCs! " + rate + ">" + RPC_CALL_TRHESHOLD + " per 100ms (" + interval + ")");
                            } else {
                                this.removePlayerData(key);
                            }
                        } else if (this.isExceedingUnexistentRPCLimit(sender.ID)) {
                            float interval = (int)(this.rpcUnexistentHistory[sender.ID].Last.Value - this.rpcUnexistentHistory[sender.ID].First.Value);
                            int rate = (int)((100F / interval) * UNEXISTENT_RPC_THRESHOLD);

                            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + sender.ID + " is sending too many Unexistent RPCs or invalid ViewIds " + rate + ">" + UNEXISTENT_RPC_THRESHOLD + " per 100ms (" + UNEXISTENT_RPC_THRESHOLD + " in " + interval + "ms)");

                            this.addToUnexistentCheck(sender.ID);

                            if (this.isExceedingUnexistendCheck(sender.ID)) {
                                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Auto-crashing player #" + sender.ID + " due to too many invalid RPCs/ViewIDs");
                                this.rpcUnexistentCheck.Clear();

                                if (!this.crasingPlayers.Contains(sender.ID)) {
                                    AoTModAPI.AoTModAPI.getModAPI().crashPlayer(sender.ID);
                                    this.crasingPlayers.Add(sender.ID);
                                }
                            }

                            if (PhotonNetwork.isMasterClient) {
                                FengGameManagerMKII.instance.kickPlayerRC(sender, true, "Too many unexisting RPCs or invalid IDs! " + rate + ">" + UNEXISTENT_RPC_THRESHOLD + " per 100ms");
                            } else {
                                this.removePlayerData(key);
                            }
                        }
                    ...
                    if ((sender != null) && !FengGameManagerMKII.ignoreList.Contains(sender.ID)) {
                        ...
                        if ((obj2 != null) && (obj2 is ExitGames.Client.Photon.Hashtable)) {
                            ...
                            if (!(hashtable[(byte)0] is int)) {
                                //AoTModAPI.AoTModAPI.getModAPI().logToConsole("Hashtable[0] is not an int");
                                return;
                            }
                            ...
                            if (hashtable.ContainsKey((byte)1)) {
                                if (!(hashtable[(byte)1] is short)) {
                                    //AoTModAPI.AoTModAPI.getModAPI().logToConsole("Hashtable[1] is not an short");
                                    return;
                                }
                            ...
                            for (short i = num6; i < hashtable.Count; i = (short)(i + 1)) {
                                //AoTModAPI.AoTModAPI.getModAPI().logToConsole("OnSerializeRead for: " + hashtable[i] + " ||| " + i);
                ...
                case 0xcb:
                    if ((sender != null) && (sender.isMasterClient && !sender.isLocal)) {
                        AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("The master client tried to kick you.");

                        if (!this.avoidingBan) {
                            this.playersWhoLeft.Clear();
                            this.playersWhoLeft.Add(PhotonNetwork.masterClient.ID);
                            this.avoidingBan = true;
                        ...
                        if (photonEvent[0xf5] is ExitGames.Client.Photon.Hashtable) {
                            ...
                            if (hashtable3[(byte)0] is int) {
                                ...
                                if ((obj3 != null) && (sender != null)) {
                                    PhotonView view = obj3.GetPhotonView();

                                    if (view != null) {
                                        if (view.isMine || view.ownerId != sender.ID) {
                                            if (PhotonNetwork.isMasterClient) {
                                                AoTModAPI.AoTModAPI.getModAPI().banName(AoTModAPI.Util.stripColorCodes((string)sender.customProperties[PhotonPlayerProperty.name]), AoTModAPI.ModBanType.FULL_NAME);
                                                FengGameManagerMKII.instance.kickPlayerRC(sender, true, "Tried to destroy other your object");
                                            }

                                            if (sender.isMasterClient) {
                                                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Master client removed your object.");
                                                this.RemoveInstantiatedGO(obj3, true);
                                            } else {
                                                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + sender.ID + " tried to delete your objects.");
                                            }
                                        } else {
                                            if (view.ownerId != sender.ID) {
                                                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player  #" + sender.ID + " destroyed an object for player #" + view.ownerId);
                                            }
                        ...
                        if (photonEvent[0xf5] is ExitGames.Client.Photon.Hashtable) {
                            ...
                            if (hashtable3[(byte)0] is int) {
                                ...
                                if(playerId != sender.ID) {
                                    AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + sender.ID + " deleted all objects owned by player #" + playerId);
                                }

                                if (playerId == PhotonNetwork.player.ID) {
                                    AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Ignoring destroy of own player objects");
                                    return;
                                }
                ...
                case 0xd0: {
                        ...
                        if (((sender != null) || !sender.isMasterClient) || (num10 != sender.ID)) {
                            if (!(((sender == null) || sender.isMasterClient) || sender.isLocal)) {
                                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + sender.ID + " tried to set the MC to #" + num10);
                ...
                case 0xfe:
                    if (key != PhotonNetwork.player.ID) {
                        if (this.avoidingBan) {
                            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Not registering leave from player #" + key + " do to you being banned.");
                            this.playersWhoLeft.Add(key);
                        } else {
            ...
            AoTModAPI.AoTModAPI.getModAPI().log("Player #" + sender.ID + " caused an exception in NetworkingPeer::OnEvent");
            this.addToUnexistentRPCHistory(sender.ID);
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + sender.ID + " caused an exception in NetworkingPeer::OnEvent");
            AoTModAPI.AoTModAPI.getModAPI().log(e);
        }
    ...
    private void OnSerializeRead(ExitGames.Client.Photon.Hashtable data, PhotonPlayer sender, int networkTime, short correctPrefix)
    {
        ...
        PhotonView photonView = this.GetPhotonView(viewID);
        //ModMain.instance.sendToPlayer("Recieved onSerializeRead for ID " + viewID + " data " + data[1]);

        if (!data.ContainsKey((byte)1)){
//ModMain.instance.logToConsole("Hashtable did not contain data.");
            return;
        }
        ...
        else if ((photonView.group == 0) || this.allowedReceivingGroups.Contains(photonView.group))
        {
            ...
            {
                //ModMain.instance.logToConsole("Observed was MonoBehaviour");
                ...
                //ModMain.instance.logToConsole("End of aodjqpojdopawjdopwjad");
            }
