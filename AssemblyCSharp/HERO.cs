...
public class HERO : Photon.MonoBehaviour {
    ...
    //Public Security mod
    public static bool modUseGas;
    public static bool modUseBlades;
    //--
    ...
    [RPC]
    private void backToHumanRPC(PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            //Dude tried to make you go back to normal
            return;
        }
        this.titanForm = false;
    ...
    public void blowAway(Vector3 force, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to add force to you.");
            return;
        }
    ...
    [RPC]
    public void hookFail(PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to cancel your hook.");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to cancel your hook.");
            }
            return;
        }
    ...
    private void killObject(PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender.ID != this.photonView.ownerId) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to destroy your HERO (killObject).");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Fake kill object.");
            }
            return;
        }

        if (info.sender != this.photonView.owner) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " destroyed player #" + this.photonView.ownerId + " HERO.");
        }
    ...
    public void loadskinRPC(int horse, string url, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender.ID != this.photonView.ownerId) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to set your skin.");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to set your skin.");
            }
    ...
    public void moveToRPC(float posX, float posY, float posZ, PhotonMessageInfo info) {
        if (info.sender.isMasterClient) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("The master client forced you to this position.");
    ...
    private void net3DMGSMOKE(bool ifON, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != this.photonView.owner) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender + " tried to toggle your gas particles.");
            return;
        }
    ...
    private void netCrossFade(string aniName, float time, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to make you play an animation.");
            return;
        }
        this.currentAnimation = aniName;
    ...
    private void escapeGrab() {
        if (this.isGrabbed) {
            this.photonView.RPC("netUngrabbed", PhotonTargets.All, null);

            if (this.titanWhoGrabMe != null) {
                this.titanWhoGrabMe.GetPhotonView().RPC("targetEscape", PhotonTargets.All, null);

                this.titanWhoGrabMe  = null;
            }
        }
    }
    ...
    public void netDie(Vector3 v, bool isBite, int viewID = -1, string titanName = "", bool killByTitan = true, PhotonMessageInfo info = null) {
        ...
        if (base.photonView.isMine) {
            ...
            if (AoTModAPI.AoTModAPI.getModAPI().isGodModeEnabled()) {
                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("[God Mode]Blocked kill from #" + info.sender.ID);
                this.escapeGrab();
                return;
            }

            if (info.sender.customProperties[PhotonPlayerProperty.name] == null || info.sender.customProperties[PhotonPlayerProperty.dead] == null || info.sender.customProperties[PhotonPlayerProperty.isTitan] == null) {
                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID + " [Invalid player properties]</color>");
                this.escapeGrab();
                return;
            }

            if (!info.sender.isLocal) {
                if (viewID < 0) {//It is an AI titan 
                    if (string.IsNullOrEmpty(titanName)) {
                        AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID + " [Null or empty titan name]</color>");
                        this.escapeGrab();
                        return;
                    }
                } else {  //Non-AI titan, (Player)
                    PhotonView view = PhotonView.Find(viewID);
                    if (view != null) {
                        if (view.ownerId != info.sender.ID) {
                            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID + " [view.ownerID != sender.ID]</color>");
                            this.escapeGrab();
                            return;
                        }

                        if (RCextensions.returnIntFromObject(info.sender.customProperties[PhotonPlayerProperty.isTitan]) != 2) {
                            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID + " [Not a titan]</color>");
                            this.escapeGrab();
                            return;
                        }
                    } else {
                        AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID + " [Invalid viewID]</color>");
                        this.escapeGrab();
                        return;
                    }
                }
        ...
        AoTModAPI.AoTModAPI.getModAPI().onPlayerDeath(this.photonView.ownerId);
        ...
        if (base.photonView.isMine) {
            ...
            PhotonNetwork.player.SetCustomProperties(propertiesToSet);
            object[] parameters = new object[] { !string.IsNullOrEmpty(titanName) ? 1 : 0 };
    ...
    private void netDie2(int viewID = -1, string titanName = "", PhotonMessageInfo info = null) {
        ...
        if (base.photonView.isMine) {
            ...
            if (!info.sender.isLocal) {
                ...
                if (viewID < 0) {//Ai titan
                    if (string.IsNullOrEmpty(titanName)) {
                        FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [Null or empty titan name].</color>");
                        this.ungrabbed();
                        return;
                    } else if (RCSettings.bombMode == 0 && RCSettings.deadlyCannons == 0){
                        if (this.titanWhoGrabMe == null) {
                            FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [NetDie 2 called without being grabbed].</color>");
                            this.ungrabbed();
                            return;
                        } else if (this.titanWhoGrabMe.GetPhotonView().ownerId != info.sender.ID) {
                            FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [Grabbed titan owner is not the same as the RPC sender].</color>");
                            this.ungrabbed();
                            return;
                        }
                    }
                    ...
                    if (view == null) {
                        FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [Invalid viewID] (PT)</color>");
                        this.ungrabbed();
                        return;
                    }

                    if (view.ownerId != info.sender.ID) {
                        FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [view.ownerID != sender.ID] (PT) </color>");
                        this.ungrabbed();
                        return;
                    }

                    if (this.titanWhoGrabMe == null) {
                        FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [HERO::netDie2 called without being grabbed] (PT)</color>");
                        this.ungrabbed();
                        return;
                    } else if (this.titanWhoGrabMe.GetPhotonView().ownerId != info.sender.ID) {
                        FengGameManagerMKII.instance.chatRoom.addLINE("<color=#FFCC00>Unusual Kill from ID " + info.sender.ID.ToString() + " [Grabbed titan owner!= sender.ID] (PT)</color>");
                        this.ungrabbed();
                        return;
                    }
                }
    ...
    public void netDieLocal(Vector3 v, bool isBite, int viewID, string titanName, bool killByTitan, bool isSuicide) {
        if (base.photonView.isMine) {
            Vector3 vector = (Vector3)(Vector3.up * 5000f);
            if (this.titanForm && (this.eren_titan != null)) {
                this.eren_titan.GetComponent<TITAN_EREN>().lifeTime = 0.1f;
    ...
    private void netGrabbed(int id, bool leftHand, PhotonMessageInfo info) {
        if (PhotonView.Find(id) == null) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to call netGrabbed with an invalid ID");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Invalid ID for netGrabbed.");
            }
            return;
        }
    ...
    private void netPauseAnimation(PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to pause your animation.");

            if (PhotonNetwork.isMasterClient){
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to pause your animation.");
            }
            return;
        }
        IEnumerator enumerator = base.animation.GetEnumerator();
    ...
    private void netPlayAnimation(string aniName, PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to make you play an animation");
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to force an animation");
            }
            return;
        }
        this.currentAnimation = aniName;
    ...
    private void netUngrabbed() {
        this.ungrabbed();
        this.photonView.RPC("netPlayAnimation", PhotonTargets.All, this.standAnimation);
        this.falseAttack();
    }
    ...
    private void RPCHookedByHuman(int hooker, Vector3 hookPosition, PhotonMessageInfo info) {
        PhotonView hookerView = PhotonView.Find(hooker); //( ͡° ͜ʖ ͡°) 
		
        this.hookBySomeOne = true;
        this.badGuy = hookerView.gameObject;
    ...
    [RPC]
    private void setMyTeam(int val, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender + " tried to set your team.");
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, false, "Tried to set your team");
            }
            return;
        }

        this.myTeam = val;
        TriggerColliderWeapon collider;

        if (this.checkBoxLeft != null){
            collider = this.checkBoxLeft.GetComponent<TriggerColliderWeapon>();
            if (collider != null) {
                collider.myTeam = val;
            }
        }
        ...
        if (this.checkBoxRight != null) {
            collider = this.checkBoxRight.GetComponent<TriggerColliderWeapon>();
            if (collider != null) {
                collider.myTeam = val;
            }
        }
    ...
    public void setTeam(int team) {
        this.setMyTeam(team, new PhotonMessageInfo());
        if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && base.photonView.isMine) {
    ...
    private void suicide2() {
        if (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE) {
            this.netDieLocal((Vector3)(base.rigidbody.velocity * 50f), false, -1, string.Empty, true, true);
    ...
    private void whoIsMyErenTitan(int id, PhotonMessageInfo info) {
		...
		if (this.photonView.isMine){
            if (info.sender.isLocal) {
                ...
                AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to set your eren titan.");
                if (PhotonNetwork.isMasterClient) {
                    FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to set your eren titan.");
                }
            }
        }else if (this.photonView.ownerId != info.sender.ID) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to set player #" + this.photonView.ownerId + " eren titan.");
        }
    }
