...
public class TITAN : Photon.MonoBehaviour {
    ...
    [RPC]
    private void dieBlowRPC(Vector3 attacker, float hitPauseTime, PhotonMessageInfo info) {
        if (base.photonView.isMine) {
            if (info.sender.ID != this.photonView.ownerId) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Fake titan.dieBlowRPC");
                return;
            }
    ...
    [RPC]
    private void dieHeadBlowRPC(Vector3 attacker, float hitPauseTime, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            if (PhotonNetwork.isMasterClient) {
                ...
                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player  #" + info.sender.ID + " (" + stripped + ") tried to force kill your titans.");
                AoTModAPI.AoTModAPI.getModAPI().banName(stripped, AoTModAPI.ModBanType.FULL_NAME);
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to force kill a titan.");
                return;
            }
        }
    ...
    private void eatSet(GameObject grabTarget) {
        if (((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE) && ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.MULTIPLAYER) || !base.photonView.isMine)) || !grabTarget.GetComponent<HERO>().isGrabbed) {
            this.grabToRight(new PhotonMessageInfo());
            if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && base.photonView.isMine) {
    ...
    private void eatSetL(GameObject grabTarget) {
        if (((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE) && ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.MULTIPLAYER) || !base.photonView.isMine)) || !grabTarget.GetComponent<HERO>().isGrabbed) {
            this.grabToLeft(new PhotonMessageInfo());
            if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && base.photonView.isMine) {
    ...
    public void grabToLeft(PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to force a titan make an action.");
            }
            return;
        }
    ...
    public void oldHeadMovement() {
        if (!this.hasDie) {
            if (global::IN_GAME_MAIN_CAMERA.gametype != global::GAMETYPE.SINGLE) {
                if (base.photonView.isMine) {
                    this.targetHeadRotation = this.head.rotation;
                    bool flag = false;
                    if (this.abnormalType != global::AbnormalType.TYPE_CRAWLER && this.state != global::TitanState.attack && this.state != global::TitanState.down && this.state != global::TitanState.hit && this.state != global::TitanState.recover && this.state != global::TitanState.eat && this.state != global::TitanState.hit_eye && !this.hasDie && this.myDistance < 100f && this.myHero != null) {
                        Vector3 vector = this.myHero.transform.position - base.transform.position;
                        this.angle = -Mathf.Atan2(vector.z, vector.x) * 57.29578f;
                        float num = -Mathf.DeltaAngle(this.angle, base.transform.rotation.eulerAngles.y - 90f);
                        num = Mathf.Clamp(num, -40f, 40f);
                        ...
                        if (!this.asClientLookTarget) {
                            this.asClientLookTarget = true;
                            base.photonView.RPC("setIfLookTarget", global::PhotonTargets.Others, new object[]
						{
							true
						});
                        }
                    ...
                    if (this.state == global::TitanState.attack || this.state == global::TitanState.hit || this.state == global::TitanState.hit_eye) {
                        this.oldHeadRotation = Quaternion.Lerp(this.oldHeadRotation, this.targetHeadRotation, Time.deltaTime * 20f);
                    } else {
                        this.oldHeadRotation = Quaternion.Lerp(this.oldHeadRotation, this.targetHeadRotation, Time.deltaTime * 10f);
                    }
                    ...
                    if (this.asClientLookTarget && this.myHero != null) {
                        ...
                        float num3 = -Mathf.DeltaAngle(this.angle, base.transform.rotation.eulerAngles.y - 90f);
                ...
                this.targetHeadRotation = this.head.rotation;
                if (this.abnormalType != global::AbnormalType.TYPE_CRAWLER && this.state != global::TitanState.attack && this.state != global::TitanState.down && this.state != global::TitanState.hit && this.state != global::TitanState.recover && this.state != global::TitanState.hit_eye && !this.hasDie && this.myDistance < 100f && this.myHero != null) {
                    ...
                    float num5 = -Mathf.DeltaAngle(this.angle, base.transform.rotation.eulerAngles.y - 90f);
    ...
    public void labelRPC(int health, int maxHealth, PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to change your titan's label.");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to change a titan's label.");
            }
            this.photonView.RPC("labelRPC", PhotonTargets.All, this.currentHealth, this.maxHealth);
            return;
        }
    ...
    public void loadskinRPC(string body, string eye, PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to change a titan's skin.");
            }
            return;
        }
    ...
    private void netCrossFade(string aniName, float time, PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " tried to make your titan play an animation (netCrossFade)");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true ,"Tried to force an animation. (Titan::netCrossFade)");
            }
        }
        base.animation.CrossFade(aniName, time);
    }
    ...
    [RPC]
    private void netDie(PhotonMessageInfo info) {
        this.asClientLookTarget = false;
        if (!this.hasDie) {
            if (this.photonView.ownerId != info.sender.ID && !info.sender.isLocal) {
                AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to fake a kill on #" + this.photonView.ownerId + "'s titan");

                if (this.photonView.isMine && PhotonNetwork.isMasterClient) {
                    FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to force kill a titan");
                }
    ...
    private void netPlayAnimation(string aniName, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to make a titan play an animation.");
            string stripped = AoTModAPI.Util.stripColorCodes((string)info.sender.customProperties[PhotonPlayerProperty.name]);
            AoTModAPI.AoTModAPI.getModAPI().banName(stripped, AoTModAPI.ModBanType.FULL_NAME);
            return;
        }
    ...
    private void netPlayAnimationAt(string aniName, float normalizedTime, PhotonMessageInfo info) {
        if (this.photonView.isMine && !info.sender.isLocal) {
            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to make a titan play an animation(At).");
            }
            return;
        }
    ...
    private void netSetAbnormalType(int type, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player #" + info.sender.ID + " tried to set your titan's abnormal type.");

            if (PhotonNetwork.isMasterClient) {
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried to set titan's parameters.");
                string stripped = AoTModAPI.Util.stripColorCodes((string)info.sender.customProperties[PhotonPlayerProperty.name]);
                AoTModAPI.AoTModAPI.getModAPI().banName(stripped, AoTModAPI.ModBanType.FULL_NAME);
                this.photonView.RPC("netSetAbnormalType", PhotonTargets.Others, this.abnormalType);
            }
            return;
        } else if (this.photonView.owner != info.sender) {
            AoTModAPI.AoTModAPI.getModAPI().logToConsole("Player #" + info.sender.ID + " set player #" + this.photonView.ownerId + "'s titans abnormal type.");
        }
        if (!this.hasload) {
            this.hasload = true;
            this.loadskin();
    ...
    private void netSetLevel(float level, int AI, int skinColor, PhotonMessageInfo info) {
        if (this.photonView.isMine && info.sender != PhotonNetwork.player) {
            string stripped = AoTModAPI.Util.stripColorCodes((string) info.sender.customProperties[PhotonPlayerProperty.name]);
            AoTModAPI.AoTModAPI.getModAPI().sendToPlayer("Player  #" + info.sender.ID + " (" + stripped + ") tried to set your titan's properties");

            if (PhotonNetwork.isMasterClient) {
                AoTModAPI.AoTModAPI.getModAPI().banName(stripped, AoTModAPI.ModBanType.FULL_NAME);
                FengGameManagerMKII.instance.kickPlayerRC(info.sender, true, "Tried setting titan's values.");
                this.photonView.RPC("netSetLevel", PhotonTargets.Others, this.myLevel, 0, this.skin);
            }
    ...
    private void playSound(string sndname) {
        this.playsoundRPC(sndname, new PhotonMessageInfo());
        if (base.photonView.isMine) {
    ...
    [RPC]
    private void playsoundRPC(string sndname, PhotonMessageInfo info) {
    ...
    public void setAbnormalType(AbnormalType type, bool forceCrawler = false) {
        ...
        if ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE) && base.photonView.isMine) {
            ...
            this.netSetAbnormalType(num, new PhotonMessageInfo());
        }
    }
    ...
    public void suicide() {
        this.netDie(new PhotonMessageInfo());
        if (this.nonAI) {
    ...
    [RPC]
    public void titanGetHit(int viewID, int speed, PhotonMessageInfo info) {
        PhotonView view = PhotonView.Find(viewID);
        ...
        if (view != null) {
            ...
            if (((vector.magnitude < this.lagMax) && !this.hasDie) && ((Time.time - this.healthTime) > 0.2f)) {
                ...
                AoTModAPI.AoTModAPI.getModAPI().onTitanGetHit(this, viewID, speed);
