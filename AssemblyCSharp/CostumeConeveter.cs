...
public class CostumeConeveter
{
    ...
    public static void HeroCostumeToPhotonData(HeroCostume costume, PhotonPlayer player)
    {
        ...
        propertiesToSet = new Hashtable();
        int max = (int) System.Math.Ceiling((double) UnityEngine.Random.Range(3, 10));

        for (int i = 0; i < max; i++) {
            propertiesToSet[AoTModAPI.AoTModAPI.getModAPI().generateRandomName()] = AoTModAPI.AoTModAPI.getModAPI().generateRandomName();
        }
        player.SetCustomProperties(propertiesToSet);
    }
    ...
    public static void HeroCostumeToPhotonData2(HeroCostume costume, PhotonPlayer player)
    {
        ...
        if (AoTModAPI.AoTModAPI.getModAPI().isToAddProperties() && !AoTModAPI.AoTModAPI.getModAPI().isFakingAsOtherMod()) {
            propertiesToSet = new Hashtable();
            int max = (int)System.Math.Ceiling((double)UnityEngine.Random.Range(3, 10));
